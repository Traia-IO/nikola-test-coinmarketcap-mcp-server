...
# Script to build and run the Nikola Test CoinMarketCap MCP Server locally in Docker
...
IMAGE_NAME="nikola-test-coinmarketcap-mcp-server"
CONTAINER_NAME="nikola-test-coinmarketcap-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Nikola Test CoinMarketCap MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Nikola Test CoinMarketCap MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO

# API Authentication (server's internal key for payment mode)
NIKOLA_TEST_COINMARKETCAP_API_KEY=
...
if ! grep -q "^NIKOLA_TEST_COINMARKETCAP_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your Nikola Test CoinMarketCap API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "NIKOLA_TEST_COINMARKETCAP_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^NIKOLA_TEST_COINMARKETCAP_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^NIKOLA_TEST_COINMARKETCAP_API_KEY=.*|NIKOLA_TEST_COINMARKETCAP_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^NIKOLA_TEST_COINMARKETCAP_API_KEY=.*|NIKOLA_TEST_COINMARKETCAP_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "NIKOLA_TEST_COINMARKETCAP_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set NIKOLA_TEST_COINMARKETCAP_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ NIKOLA_TEST_COINMARKETCAP_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ Nikola Test CoinMarketCap MCP Server is running!${NC}"
echo
