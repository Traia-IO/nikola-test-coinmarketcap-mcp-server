# Nikola Test CoinMarketCap MCP Server

This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Nikola Test CoinMarketCap API. It enables AI agents and LLMs to interact with Nikola Test CoinMarketCap through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Nikola Test CoinMarketCap endpoints
...
- **Nikola Test CoinMarketCap Website**: [https://pro-api.coinmarketcap.com](https://pro-api.coinmarketcap.com)
...
*Note: Replace `example_tool` with actual Nikola Test CoinMarketCap API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/nikola-test-coinmarketcap-mcp-server.git
   cd nikola-test-coinmarketcap-mcp-server
   ```

2. Set your API key:
   ```bash
   export NIKOLA_TEST_COINMARKETCAP_API_KEY="your-api-key-here"
   ```
...
# Server's internal API key (for payment mode)
   NIKOLA_TEST_COINMARKETCAP_API_KEY=your-api-key-here
...
2. Run the server:
   ```bash
NIKOLA_TEST_COINMARKETCAP_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own Nikola Test CoinMarketCap API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_NIKOLA_TEST_COINMARKETCAP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their Nikola Test CoinMarketCap API key
2. Server uses client's API key to call Nikola Test CoinMarketCap API
3. No payment required
4. Client pays Nikola Test CoinMarketCap directly
...
6. Server uses its INTERNAL Nikola Test CoinMarketCap API key to call the API
...
```bash
# Required
NIKOLA_TEST_COINMARKETCAP_API_KEY=your_internal_nikola-test-coinmarketcap_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Nikola Test CoinMarketCap API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Nikola Test CoinMarketCap endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/nikola-test-coinmarketcap-mcp-server",
  "mcp_server": {
    "name": "nikola-test-coinmarketcap-mcp",
    "description": "The coinmarketcap api is a restful json api that provides programmatic access to comprehensive cryptocurrency market intelligence, including real-time and historical prices, market capitalization, trading volume, ohlcv time-series data, global market metrics (such as total market cap and bitcoin dominance), exchange and market-pair data, and rich metadata for thousands of digital assets. authenticated via api keys and offered across free to enterprise tiers, it is designed for reliability and scale, making it suitable for applications such as price tracking dashboards, portfolio management tools, trading and analytics systems, quantitative research and backtesting, defi and exchange monitoring, data pipelines, and media or educational platforms that require accurate, up-to-date crypto market data.",
...
"capabilities": [
  ...
  "tags": ["nikola test coinmarketcap", "api"],
  "ref": "main"
}
```
...
- `NIKOLA_TEST_COINMARKETCAP_API_KEY`: Your Nikola Test CoinMarketCap API key (required)
## Troubleshooting
