# Nikola Test CoinMarketCap MCP Server Implementation Guide

You are working on implementing the Nikola Test CoinMarketCap MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: Nikola Test CoinMarketCap
...
- **Authentication**: Required - API key via `NIKOLA_TEST_COINMARKETCAP_API_KEY` environment variable
...
When a client provides their own Nikola Test CoinMarketCap API key:
...
Flow:
  1. Client connects with their Nikola Test CoinMarketCap API key
  2. MCP server uses client's API key to call Nikola Test CoinMarketCap API
  3. No payment required
  4. Client pays Nikola Test CoinMarketCap directly (not the MCP server)
```

**Use Case**: Clients who already have a Nikola Test CoinMarketCap subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL Nikola Test CoinMarketCap API key to call the API
  5. Client pays the MCP server (not Nikola Test CoinMarketCap)
```

**Use Case**: Pay-per-use clients without their own Nikola Test CoinMarketCap subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("NIKOLA_TEST_COINMARKETCAP_API_KEY")
...
**Required**:
- `NIKOLA_TEST_COINMARKETCAP_API_KEY`: Server's internal Nikola Test CoinMarketCap API key (used when clients pay via 402)
...
NIKOLA_TEST_COINMARKETCAP_API_KEY=your_nikola-test-coinmarketcap_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_nikola_test_coinmarketcap",
      "get_nikola_test_coinmarketcap_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["nikola test coinmarketcap", "api", /* add relevant tags like "search", "data", etc. */]
}
```

### 2. Study the API Documentation

First, thoroughly review the API documentation at https://coinmarketcap.com/api/documentation/v1/ to understand:
- Available endpoints
...
Add functions to call the Nikola Test CoinMarketCap API with retry support. Example pattern:
...
def call_nikola_test_coinmarketcap_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call Nikola Test CoinMarketCap API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"Nikola Test CoinMarketCap API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_nikola_test_coinmarketcap(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search Nikola Test CoinMarketCap for [specific data type].
    ...
    try:
        # The call_nikola_test_coinmarketcap_api function already has retry logic
        results = call_nikola_test_coinmarketcap_api(
...
```python
@mcp.tool()
async def get_nikola_test_coinmarketcap_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_nikola_test_coinmarketcap_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in Nikola Test CoinMarketCap.
...
- **Log retry attempts** for debugging:
  ...
  def call_nikola_test_coinmarketcap_api(...):
      # Implementation
  ```
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_nikola_test_coinmarketcap(query="example", limit=5)
   ```
...
### 9. Common Nikola Test CoinMarketCap Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on Nikola Test CoinMarketCap capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the Nikola Test CoinMarketCap documentation that you should implement:
...
- Check the Nikola Test CoinMarketCap API documentation: https://coinmarketcap.com/api/documentation/v1/
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make Nikola Test CoinMarketCap's capabilities easily accessible to AI agents through standardized MCP tools. 